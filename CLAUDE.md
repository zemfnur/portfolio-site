...
- `about.html` — the About page (implements Figma "About-1920"). Everything translates: keys `aboutTitle`, `aboutIntro`, `aboutBody1`–`aboutBody5` (plain text) plus `aboutQuote` and `aboutList` (swap as HTML — the quote carries a `<br>` line break, the list its dash lines). `.about__title`/`.about__quote` use `var(--font-display)`, so RU renders in Oswald (Barlow Semi Condensed has no Cyrillic; the page loads Oswald for this, like `index.html`). The mockup shows RU copy only; the EN text in the markup is a translation.
- `projects/*.html` — case-study pages (`logistics-crm.html` implements Figma "Portfolio-case1", `production-line-simulator.html` implements "Portfolio-2", `martin-exam.html` implements "Portfolio-case-3"). All share the case-study component styles in `styles.css` (`.case-hero`, `.scenario`, `.result`, …; case 3's sections are tight 80px `.band`s with hairlines rather than 200px `.case-section`s). i18n keys in `main.js` are global: case-1 keys are unprefixed (`caseTitle`…), case-2 keys are prefixed (`case2…`, `c2…`, `m…`, `uf…`), case-3 keys are prefixed `case3…`/`c3…`. Case 3's "What I designed" cards 02–06 intentionally render empty white screenshot slots — the Figma frame leaves them unfilled.
